...
import static utilz.Constants.PlayerConstants.ATTACK_2;
...
public class Player extends Entity{
	...
	public void render(Graphics g) {
		...
		g.drawImage(animations[playerAction][aniIndex], (int) x, (int) y,720,300, null);
...
animations = new BufferedImage[8][10];
		...
		for(int j = 0; j < animations.length; j++) 
		    ...
		    for(int i = 0; i < animations[j].length; i++)
			    animations[j][i] = img.getSubimage(i*120, j*51, 120, 50);
	} catch (IOException e) {
		e.printStackTrace();
	} finally {
		try {
